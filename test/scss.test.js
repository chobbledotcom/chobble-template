...
const testCases = [
  {
    ...
    test: () => {
      ...
      expectTrue(result.includes('color: red'), "Should compile SCSS variables");
  ...
  },
  {
    ...
    test: () => {
      ...
      const scssWithImports = '@import "variables"; body { background: $bg-color; }';
      ...
      try {
        ...
        expectTrue(error.message.includes('Can\'t find stylesheet') || 
  ...
  },
  {
    ...
    test: () => {
      ...
      expectTrue(result.includes('color: #333') || result.includes('color:#333'), "Should compile SCSS variables");
    }
  },
  {
    ...
    test: () => {
      ...
      expectTrue(result.includes('.nav ul li'), "Should handle deeply nested selectors");
    }
  },
  {
    ...
    test: () => {
      ...
      expectTrue(result.includes('background: blue') || result.includes('background:blue'), "Should apply mixin");
      expectTrue(result.includes('padding: 10px') || result.includes('padding:10px'), "Should include mixin properties");
    }
  },
  {
    ...
    test: () => {
      ...
      expectArrayLength(mockConfig.templateFormats, 1, "Should add template format");
      expectStrictEqual(mockConfig.templateFormats[0], 'scss', "Should add scss format");
      
      expectTrue(mockConfig.extensions.scss !== undefined, "Should add scss extension");
      
      const scssExtension = mockConfig.extensions.scss;
      expectStrictEqual(scssExtension.outputFileExtension, 'css', "Should output CSS files");
      expectFunctionType(scssExtension, 'compile', "Should have compile function");
    }
  },
  {
    ...
    test: () => {
      ...
      expectFunctionType(compileFn, undefined, "Compile should return a function");
      
      const result = compileFn({});
      expectTrue(typeof result === 'string', "Should return compiled CSS string");
      expectTrue(result.includes('.test'), "Should include CSS selector");
      expectTrue(result.includes('color: green') || result.includes('color:green'), "Should compile SCSS");
    }
  },
  {
    ...
    test: () => {
      ...
      expectTrue(typeof result === 'string', "Should compile successfully with load paths");
  ...
  },
  {
    ...
    test: () => {
      ...
      expectStrictEqual(contentCopy, originalContent, "Should not modify input content");
