...
const testCases = [
  ...
  },
  {
    ...
    test: () => {
      ...
      assert.strictEqual(result[0].data.header_image, 'widget-final.jpg', "Should assign widget image");
      assert.strictEqual(result[1].data.header_image, 'gadget-final.jpg', "Should assign gadget image");
      assert.strictEqual(result[0].data.title, 'Widgets', "Should preserve other category data");
    }
  },
  {
    ...
    test: () => {
      ...
      assert.strictEqual(result[0].data.header_image, undefined, "Should assign undefined for missing mapping");
    }
  },
  {
    ...
    test: () => {
      ...
      assert.strictEqual(result[0], categories[0], "Should return same object reference");
      assert.strictEqual(categories[0].data.header_image, 'widget.jpg', "Should mutate original object");
    }
  },
  {
    ...
    test: () => {
      ...
      assert.deepStrictEqual(result, [], "Should return empty array for no categories");
    }
  },
  {
    ...
    test: () => {
      const mockCollectionApi = {
        getFilteredByTag: (tag) => {
          if (tag === 'category') {
            return [
              { fileSlug: 'widgets', data: { title: 'Widgets', header_image: 'default-widget.jpg' } },
              { fileSlug: 'gadgets', data: { title: 'Gadgets' } }
            ];
          }
          if (tag === 'product') {
      ...
      assert.strictEqual(result[0].data.header_image, 'premium-widget.jpg', "Should use high-order product image for widgets");
      assert.strictEqual(result[1].data.header_image, 'basic-gadget.jpg', "Should use product image for gadgets");
      assert.strictEqual(result[0].data.title, 'Widgets', "Should preserve category title");
    }
  },
  {
    ...
    test: () => {
      ...
      assert.strictEqual(result.length, 1, "Should return only featured categories");
      assert.strictEqual(result[0].data.title, 'Featured Category', "Should return the featured category");
    }
  },
  {
    ...
    test: () => {
      assert.deepStrictEqual(getFeaturedCategories(null), [], "Should return empty array for null");
      assert.deepStrictEqual(getFeaturedCategories(undefined), [], "Should return empty array for undefined");
    }
  },
  {
    ...
    test: () => {
      ...
      assert(mockConfig.collections.categories, "Should add categories collection");
      assert.strictEqual(typeof mockConfig.collections.categories, 'function', "Should add function collection");
      assert.strictEqual(mockConfig.collections.categories, createCategoriesCollection, "Should use correct collection function");
      
      assert(mockConfig.filters.getFeaturedCategories, "Should add getFeaturedCategories filter");
      assert.strictEqual(typeof mockConfig.filters.getFeaturedCategories, 'function', "Should add function filter");
      assert.strictEqual(mockConfig.filters.getFeaturedCategories, getFeaturedCategories, "Should use correct filter function");
    }
  },
  {
  ...
  },
  {
    ...
    test: () => {
      const originalCategories = [
        { fileSlug: 'widgets', data: { title: 'Widgets', header_image: 'original.jpg' } }
      ];
      ...
      assert.deepStrictEqual(categoriesCopy, originalCategories, "Should not modify categories input");
      assert.deepStrictEqual(productsCopy, originalProducts, "Should not modify products input");
      
      // Verify function is pure (same inputs = same outputs)
      assert.deepStrictEqual(result1, result2, "Should produce identical results for same inputs");
      
      // Verify results are new objects
      assert.notStrictEqual(result1, result2, "Should create new result objects each time");
    }
  },
  {
  ...
  },
  {
    ...
    test: () => {
      ...
      assert.deepStrictEqual(categoriesCopy, originalCategories, "buildCategoryImageMap should not modify categories");
      assert.deepStrictEqual(productsCopy, originalProducts, "buildCategoryImageMap should not modify products");
    }
  }
];
