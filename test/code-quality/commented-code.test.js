...
// ============================================
// Commented Code Checker Factory
// Consolidates detection of commented-out code
// ============================================

/**
 * Factory that creates a commented code checker with methods for:
 * - Checking if code is inside template literals
 * - Checking if comments are documentation
 * - Finding commented-out code in source
 * - Analyzing files for violations
 */
const createCommentedCodeChecker = (jsFiles, config = {}) => {
  const {
    excludeFiles = [],
  ...
  const isDocumentationComment = (_line, _prevLine, nextLine) => {
    if (nextLine && /^\s*\/[^/]/.test(nextLine)) return true;
    return false;
  };
  ...
  const findCommentedCode = (source, _relativePath) => {
    ...
    return lines
      .filter(({ line }, i) => {
        ...
        return commentedCodePatterns.some(
          (pattern) =>
            pattern.test(line) &&
...
const commentedCodeChecker = createCommentedCodeChecker(
...
const c = 3;
    `;
    const results = commentedCodeChecker.findCommentedCode(source, "test.js");
...
const a = 1;
  ...
  test("No commented-out code allowed in the codebase", () => {
    const violations = commentedCodeChecker.analyzeCommentedCode();
