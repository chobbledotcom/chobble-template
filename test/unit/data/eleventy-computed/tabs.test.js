...
describe("eleventyComputed.tabs", () => {
  test("returns the tab list intact when every tab already has a body", () => {
    const tabs = [{ name: "Tab1", body: "Content1" }];
    expect(eleventyComputed.tabs({ tabs })).toEqual([
      { name: "Tab1", body: "Content1" },
    ]);
  });
  ...
  test.each([
    ["undefined", { name: "Tab1" }],
    ["null", { name: "Tab1", body: null }],
    ["empty string", { name: "Tab1", body: "" }],
  ...
  test("preserves non-body tab properties when defaulting body", () => {
    const tabs = [{ name: "Tab1", image: "image.jpg" }];
    expect(eleventyComputed.tabs({ tabs })).toEqual([
      { name: "Tab1", image: "image.jpg", body: "" },
    ]);
  });
});
